...
pub fn verify_signature(data: &[u8], public_key: &[u8], data_signature: &[u8]) -> Result<()> {
...
pub fn create_signature(data: &[u8], private_key: &[u8]) -> Result<Vec<u8>> {
...
pub fn create_public_key(private_key: &[u8]) -> Result<Vec<u8>> {
    let key_pair = signature::Ed25519KeyPair::from_pkcs8(private_key)?;
