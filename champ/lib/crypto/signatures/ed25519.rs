...
pub fn verify_signature(data: &[u8], public_key: &[u8], data_signature: &[u8]) -> Result<(), Ed25519Error> {
    let verification_key = VerificationKey::try_from(public_key).map_err(|_| Ed25519Error::VerificationError)?;
    let sig_bytes: [u8; 64] = data_signature.try_into().map_err(|_| Ed25519Error::VerificationError)?;
    verification_key.verify(&sig_bytes.into(), data).map_err(|_| Ed25519Error::VerificationError)?;
    Ok(())
}

// /// create signature from data
pub fn create_signature(data: &[u8], private_key: &[u8]) -> Result<[u8; 64], Ed25519Error> {
    let signing_key = SigningKey::try_from(private_key).map_err(|_| Ed25519Error::VerificationError)?;
    let sig_bytes: [u8; 64] = signing_key.sign(data).into();
    Ok(sig_bytes)
}
...
pub fn generate_private_key() -> Result<[u8; 32], Ed25519Error> {
    ...
    let priv_key: [u8; 32] = signing_key.into();
    Ok(priv_key)
}
...
pub fn create_public_key(private_key: &[u8]) -> Result<[u8; 32], Ed25519Error> {
    let signing_key = SigningKey::try_from(private_key).map_err(|_| Ed25519Error::VerificationError)?;
    let pk: [u8; 32] = VerificationKey::from(&signing_key).into();
    Ok(pk)
}
